...
`inpaint_map` also carries **`Cloud Cleaner`** (`modules/inpainting/remote.py`), which is not a local model at all: it posts the page and its mask to a RunPod serverless endpoint the user supplies in `Settings > Credentials > Cloud Cleaner`, and RunPod bills them directly per GPU-second. Three things about it are load-bearing. It sets **`force_full_image_inpainting`**, a flag `pipeline/inpainting.py` already read and no local engine set, which sends cleaning down `_inpaint_full_image` (one call per page) instead of `_inpaint_by_patches` (one call per merged mask region) — a page merges into roughly eight regions, and RunPod bills queue plus execution per job, so the flag is the difference between one billed job and eight. The **mask goes as PNG** while the page goes as JPEG, because a JPEG'd binary mask picks up ringing that the server reads as a partial instruction and smears every cleaned boundary. And because inpainters are constructed as `InpainterClass(device, backend=...)` with nothing else, the endpoint and key are fetched in `_ensure_inpainter` on the Qt side and passed in as kwargs — the engine must not read settings itself or it would import Qt and break the headless gate. The endpoint is part of the engine's cache key, so re-pointing it rebuilds rather than quietly billing the old one. `runpod/serverless_handler.py` is the matching worker and imports this repo's own `lama.py` so the halves cannot drift; `tests/test_cloud_cleaner_contract.py` runs both against each other.
