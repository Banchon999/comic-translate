...
**Text layers are exported but do not render in Photopea, and that is still open.** PhotoshopAPI writes a type layer with bounds `(0,0,0,0)` and **no colour channels** — `TextLayer_8bit` has no parameter for pixel data, and passing `width`/`height`/`pos_x`/`pos_y` only sets a bbox (with *centre* semantics, unlike `position_x`/`position_y`, which are top-left) while the channel count stays zero. Photopea composites a type layer from its cached raster, so it draws nothing on load, while still listing the layer and reading its string back — which is why it looks fine in a layers panel. Writing the same string back through Photopea's own API forces a re-layout and the glyphs appear. Two red herrings were ruled out by experiment rather than argument: the EngineData also contains two empty containers written without inner whitespace (`<<>>` and `[]`) that psd-tools refuses to tokenise, but byte-patching them to `<< >>` and `[ ]` leaves Photopea's composite **bit-for-bit identical**; and fill opacity is already 1.0 on every layer. `tests/test_psd_in_photopea.py` carries this as a `strict=True` xfail, so it turns into a failure the moment it is fixed.
