...
`tests/test_psd_export.py` deliberately reads exported files back with **psd-tools** rather than PhotoshopAPI, which wrote them: asserting with the writer only proves it agrees with itself, and the first two PSD bugs found (a black merged-image section, and a frozen build dying before writing anything) were invisible that way. The third one, layers written as ZipPrediction so that every reader without Zip support saw an empty canvas, was invisible even to psd-tools — it decodes Zip perfectly well — so `test_layer_pixels_use_a_compression_every_reader_implements` reads the compression code out of the raw bytes instead. The rule underneath all three: PhotoshopAPI's defaults are chosen for Photoshop, and Photoshop is the one reader that never exercises them. psd-tools is a dev dependency and cannot replace PhotoshopAPI for export — it creates pixel layers and groups but has no way to author an editable text layer.
