...
class LazyImageLoader:
    ...
    def _load_single_page(self, page_idx: int):
        ...
        if (
            page_idx in self.loaded_pages
            or page_idx in self.loading_pages
            or not (0 <= page_idx < len(self.image_file_paths))
        ):
            return
    ...
    def _adjust_scene_items_for_layout_change(self, changed_page_idx: int, height_diff: int):
        ...
        # image_positions already includes the height adjustment, so shift the
        # threshold back to the pre-adjustment page boundary before deciding
        # which existing scene items still need to move.
    ...
    def _unload_page(self, page_idx: int):
        ...
        # Persist scene items while the page's actual image width is still
        # available. Otherwise scene_to_page_local_position falls back to
        # webtoon_width and stores incorrect local X coordinates.
        ...
        # Remove image data from memory only after scene items were serialized
    ...
    def get_visible_area_image(self, paint_all=False, include_patches=True) -> tuple[np.ndarray, list]:
        ...
        visible_page_indices = sorted(self.layout_manager.get_pages_for_scene_bounds(vp_rect))
        for i in visible_page_indices:
            y = self.layout_manager.image_positions[i]
            h = self.layout_manager.image_heights[i]
    ...
    def remove_pages(self, file_paths_to_remove: list[str]) -> bool:
        ...
        try:
            path_to_index = {
                file_path: index for index, file_path in enumerate(self.image_file_paths)
            }
            ...
            # Persist only the loaded pages; unloaded pages already live in image_states.
            self.scene_item_manager.save_loaded_scene_items_to_states()
            self.viewer.setUpdatesEnabled(False)
            try:
                for page_idx in indices_to_remove:
                    self._remove_page_visuals(page_idx)
                ...
                self.loaded_pages = {
                    idx for idx in (
                        ...
                        for old_idx in self.loaded_pages
                    ) if idx is not None
                }
                self.loading_pages = {
                    idx for idx in (
                ...
                if self.image_file_paths:
                    ...
                    for page_idx in sorted(self.loaded_pages):
                        if page_idx < len(self.image_file_paths):
    ...
    def _remove_page_visuals(self, page_idx: int):
        self.loaded_pages.discard(page_idx)
        self.loading_pages.discard(page_idx)

        item = self.image_items.pop(page_idx, None)
        if item is not None:
            self._scene.removeItem(item)

        self.image_data.pop(page_idx, None)

        placeholder = self.placeholder_items.pop(page_idx, None)
        if placeholder is not None:
            self._scene.removeItem(placeholder)

    def _remap_indexed_dict(self, items: dict[int, object], removed_indices: list[int]) -> dict[int, object]:
        remapped: dict[int, object] = {}
        for old_idx, value in items.items():
