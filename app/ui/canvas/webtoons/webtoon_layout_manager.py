    ...
    def scene_to_page_coordinates(self, scene_pos: QPointF) -> Tuple[int, QPointF]:
        ...
        if 0 <= page_index < len(self.image_positions):
            if self.coordinate_converter is not None:
                # Pages are centred on the strip; only the converter knows each
                # page's width, so it owns the x offset.
                return page_index, self.coordinate_converter.scene_to_page_local_position(
                    scene_pos, page_index
                )
            page_y = self.image_positions[page_index]
            return page_index, QPointF(scene_pos.x(), scene_pos.y() - page_y)
